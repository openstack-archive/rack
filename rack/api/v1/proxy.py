...
class Controller(wsgi.Controller):
    ...
    def show(self, req, gid):
        ...
        try:
            ...
            proxy = db.process_get_all(context, gid, {"is_proxy":True, "status":"ACTIVE"})
            if len(proxy) == 0:
    ...
    def create(self, req, body, gid):
        ...
        def _validate_metadata(args):
            if args is None:
                return dict(roles="ipc/shm/proxy")
            else:
        ...
        def _get_proxy_conf():
            ...
            conf.update(dict(rabbit_host=re.sub(expr, cfg.CONF.my_ip, cfg.CONF.rabbit_host)))
            conf.update(dict(sql_connection=re.sub(expr, cfg.CONF.my_ip, cfg.CONF.sql_connection)))
            return conf
        ...
        try:
            ...
            proxy = db.process_create(context, values, network_ids, securitygroup_ids)
        ...
        try:
            ...
            self.operator_rpcapi.process_create(
                        ...
                        neutron_securitygroup_ids=[securitygroup["neutron_securitygroup_id"] for securitygroup in proxy["securitygroups"]],
                        neutron_network_ids=[network["neutron_network_id"] for network in proxy["networks"]],
                        metadata=metadata,
