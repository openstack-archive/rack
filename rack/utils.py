...
notify_decorator = 'rack.notifications.notify_decorator'
...
utils_opts = [
    ...
    cfg.StrOpt('rootwrap_config',
               default="/etc/rack/rootwrap.conf",
...
def vpn_ping(address, port, timeout=0.05, session_id=None):
    ...
    if len(received) != struct.calcsize(fmt):
        LOG.warning(_('Expected to receive %(exp)s bytes, but actually %(act)s') %
...
def _get_root_helper():
    return 'sudo rack-rootwrap %s' % CONF.rootwrap_config
...
def rackdir():
    import rack
    return os.path.abspath(rack.__file__).split('rack/__init__.py')[0]
    ...
    try:
        ...
        for match in re.finditer(route, out[0]):
            ...
            if (match.group(3) == iface and
                ...
                except exception.RackException:
                    pass
...
def monkey_patch():
    ...
    Example:
      'rack.api.ec2.cloud:rack.notifications.notify_decorator'

    Parameters of the decorator is as follows.
    (See rack.notifications.notify_decorator)
...
def generate_mac_address():
    ...
    #             properly: 0xfa.
    #             Discussion: https://bugs.launchpad.net/rack/+bug/921838
    mac = [0xfa, 0x16, 0x3e,
...
def is_neutron():
    ...
    try:
        ...
        if cls_name == 'rack.network.quantumv2.api.API':
            cls_name = 'rack.network.neutronv2.api.API'

        from rack.network.neutronv2 import api as neutron_api
