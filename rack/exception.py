...
class RackException(Exception):
    ...
    def format_message(self):
        ...
        # which should be our full RackException message, (see __init__)
        return self.args[0]


class EncryptionFailure(RackException):
    msg_fmt = _("Failed to encrypt text: %(reason)s")


class DecryptionFailure(RackException):
    msg_fmt = _("Failed to decrypt text: %(reason)s")


class VirtualInterfaceCreateException(RackException):
    msg_fmt = _("Virtual Interface creation failed")


class VirtualInterfaceMacAddressException(RackException):
...
class ImageNotActive(RackException):
...
class ImageNotAuthorized(RackException):
    msg_fmt = _("Not authorized for image %(image_id)s.")


class Invalid(RackException):
...
class VolumeNotCreated(RackException):
...
class HypervisorUnavailable(RackException):
...
class ComputeServiceInUse(RackException):
...
class ConstraintNotMet(RackException):
    msg_fmt = _("Constraint not met.")
    code = 412


class NotFound(RackException):
...
class AgentBuildExists(RackException):
...
class ImageNotFoundEC2(ImageNotFound):
    msg_fmt = _("Image %(image_id)s could not be found. The rack EC2 API "
...
class NetworkInUse(RackException):
    msg_fmt = _("Network %(network_id)s is still in use.")


class NetworkNotCreated(RackException):
...
class NoMoreNetworks(RackException):
...
class FixedIpExists(RackException):
...
class FixedIpAlreadyInUse(RackException):
    msg_fmt = _("Fixed IP address %(address)s is already in use on instance "
                "%(instance_uuid)s.")


class FixedIpAssociatedWithMultipleInstances(RackException):
...
class NoMoreFixedIps(RackException):
    ec2_code = 'UnsupportedOperation'
...
class FloatingIpExists(RackException):
...
class FloatingIpMultipleFoundForAddress(RackException):
...
class FloatingIpAssociated(RackException):
    ec2_code = "UnsupportedOperation"
    msg_fmt = _("Floating ip %(address)s is associated.")


class FloatingIpNotAssociated(RackException):
...
class CannotDisassociateAutoAssignedFloatingIP(RackException):
    ec2_code = "UnsupportedOperation"
...
class ServiceBinaryExists(RackException):
    msg_fmt = _("Service with host %(host)s binary %(binary)s exists.")


class ServiceTopicExists(RackException):
...
class QuotaExists(RackException):
...
class NoUniqueMatch(RackException):
...
class ConsolePoolExists(RackException):
...
class ConsolePortRangeExhausted(RackException):
...
class CellExists(RackException):
    msg_fmt = _("Cell with name %(name)s already exists.")


class CellRoutingInconsistency(RackException):
...
class SecuritygroupNotFound(NotFound):
    msg_fmt = _("Security group %(securitygroup_id)s not found.")


class SecuritygroupCreateFailed(RackException):
    msg_fmt = _("Unable to create Securitygroup")


class SecuritygroupDeleteFailed(RackException):
    msg_fmt = _("Unable to delete Securitygroup")


class SecuritygroupruleNotFound(NotFound):
    msg_fmt = _("Securitygrouprule %(rule_id)s could not be found.")


class SecuritygroupruleCreateFailed(RackException):
    msg_fmt = _("Unable to create Securitygrouprule")


class SecuritygroupruleDeleteFailed(RackException):
    msg_fmt = _("Unable to delete Securitygrouprule")


class SecuritygroupInUse(RackException):
    msg_fmt = _("Securitygroup %(securitygroup_id)s is still in use.")


class ProcessCreateFailed(RackException):
    msg_fmt = _("Unable to create Process")


class ProcessDeleteFailed(RackException):
    msg_fmt = _("Unable to delete Process")
