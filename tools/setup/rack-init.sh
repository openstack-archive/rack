...
RACK_CONF="/root/rack/etc/rack.conf"
RACK_CLIENT="/root/rack/tools/rack_client.py"
...
echo "api_paste_config = /root/rack/etc/api-paste.ini" >> $RACK_CONF
...
if [ 1 -eq $ipc ]; then
  ...
  python /root/websocket_server.py $WS_PORT &
fi
if [ 1 -eq $shm ]; then
...
if [ 1 -eq $proxy ]; then
  ...
  python $RACK_CLIENT proxy-update --gid $gid --shm_endpoint {\"host\":\"$host\"\,\"port\":\"$shm_port\"} --ipc_endpoint {\"host\":\"$host\"\,\"port\":\"$WS_PORT\"} --app_status "ACTIVE"
fi
