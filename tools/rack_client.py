...
def get_parser():
    ...
    proxy_create_parser.add_argument("--metadata", metavar="key1=value1[,key2=value2...]", action="store")
    proxy_create_parser.add_argument("--args", metavar="val1,[val2...]", action="store")
...
def proxy_create(args, headers):
    ...
    if args.userdata:
        payload["userdata"] = args.userdata
    if args.args:
        payload["args"] = {"roles":args.args}
    else:
        payload["args"] = {}
    if args.metadata:
        payload["args"].update(dict(v.split("=", 1) for v in args.metadata.split(",")))
